---
title: "Simuação de Eventos Discretos"
...
* Assumimos:
    ...
    9. O objetvio da simulação é determinar o valor esperado do tempo, $E[T]$;

* Variáveis: 
    1. **Tempo** $t$;
    2. **Estado** do Sistema $r$, númeor de maquinas no reparo
    3. **Eventos** $t_1 \le t_2 \le \dots \le t_n$ tempos em que houveram falhas e $t^*$ tempo para o termino do próximo reparo;
...
- **Se** $t_1 \textless t^*$ (uma nova falha)    
  ...
  \vspace{12mm} e ordene $t_2, t_3, \dots, t_n, t + X$;  
  se $r = 1$, gere $R \sim G$, e reinicie $t^* = t + R$;  
- **Senão** (um reparo terminado)  
  reinicie $t = t^*$, $r = r - 1$;  
  \vspace{12mm} caso contrário, $t^* = \infty$.  
